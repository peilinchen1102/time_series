...
def main(X_train, y_train, X_test, y_test, window_size=None, overlap=10) -> None:
    ...
    model_inputs = []
    for time_series in X_train: # (19000, 1000, 12)
        change_points = detect_changes(time_series)
        tokens = tokenize(torch.from_numpy(time_series).to(device), window_size, overlap, change_points)
        pos_encoding = positional_encoding(tokens.shape[0], tokens.shape[1]).unsqueeze(-1).to(device)
    ...
    padded_input_seq = padded_input_seq.float().to(device)

    dataset = PTBXL(padded_input_seq, y_train.to(device))
    dataloader = DataLoader(dataset, batch_size=32, shuffle=True)
    
    batch_size, seq_len, window_size, channels = X_train.shap
    ...
    test_loader = DataLoader(PTBXL(X_test, y_test), batch_size=batch_size, shuffle=False)
...
if __name__ == "__main__":
    ...
    y_test = y_test[mask]
    X_test = X_test[mask]
    y_test = torch.tesnor(y_test.cat.codes.values, dtype=torch.long)
