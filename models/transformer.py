...
class ImageNetVisionTransformer(BaseModel):
    '''
    ImageNet21k Pretrained ViT-T
    Source: https://github.com/rwightman/pytorch-image-models/blob/main/timm/models/vision_transformer.py
    Reproduce timm ViT architecture to be compatible with BaseModel specifications 
    and ImageNet pretrained checkpoint.
    '''

    def __init__(self, input_specs: List[Dict[str, Any]], in_channels=3):
        # See src/encoders/base.py for embedding modules.
        super().__init__(input_specs=input_specs)

        self.model = timm.create_model('vit_tiny_patch16_224_in21k', pretrained=True)
        self.emb_dim = self.model.embed_dim
        self.pos_embedding = self.model.pos_embed
        self.cls_token = self.model.cls_token
        self.dropout = self.model.pos_drop
        self.norm_pre = nn.Identity()
        self.transformer = self.model.blocks
        self.to_latent = self.model.norm

        # Classifier Head
        self.fc_norm = self.model.fc_norm
        self.head = self.model.head

        # True if input is one-channel
        self.expand_channels = in_channels != 3
    ...
    def encode(self, x: torch.tensor, prepool=False, prehead=False):
        ...
        if self.model.global_pool:
            x = x[:, self.model.num_prefix_tokens:].mean(dim=1) if self.model.global_pool == 'avg' else x[:, 0]
